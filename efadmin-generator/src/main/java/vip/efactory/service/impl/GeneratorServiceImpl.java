    ...
    public void generator(List<ColumnInfo> columnInfos, GenConfig genConfig, String tableName) {
        ...
        try {
            ...
            if (list.size() == 1) {
                ...
                TableInfo tableInfo = new TableInfo(obj[0], obj[1], obj[2], obj[3], ObjectUtil.isNotEmpty(obj[4]) ? obj[4] : "-");
                GenUtil.generatorCode(columnInfos, genConfig, tableInfo);
            } else {
                throw new RuntimeException("数据库中该表名" + tableName + "数量异常!");
            }
